...
if(isset($_GET["page_next"])){
  $index_next=$_GET["page_next"]+1;
}else{
  $index_next=1;
}else if(issets($_GET["page_prve"]))
...
function show_list($arr){
  ...
  if(count($arr)>0){  
    ...
    echo '</li>';
      ...
      echo '   <a class="page" href="./filter.php?page_prve='.$index_prve.'" class="next"><ion-icon name="chevron-back-outline"></ion-icon></a>';
      if(count($arr)>=10){
