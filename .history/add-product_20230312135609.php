...
function confirm_change_info_product(){
    global $conn;
    var_dump($_GET["sort_product_detail"]);
    if(isset($_POST["change_info_product"])){
       $id_product=$_POST["id-product"];
       $arr_img=$conn->query("SELECT SP_IMG_1,SP_IMG_2 FROM `SANPHAM` WHERE SP_ID=$id_product")->fetch_assoc();
       ...
       $img_1=$_FILES["img-1"];
       $img_2=$_FILES["img-2"];
       $cate_id=$_POST["id_cate"];
       $id_room=$_POST["id_rooms"];
       ...
       if($change_info_ok){
        $sql_insert_product="UPDATE `sanpham` SET `DM_ID` = '8', `SP_TEN` = 'Ghế phòng khách', `SP_GIA` = '1323',
         `SP_IMG_1` = 'eqwe', `SP_IMG_2` = '2aasdad', `SP_DES` = 'đá' WHERE `sanpham`.`SP_ID` = 22";
       }
