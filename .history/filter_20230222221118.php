...
if(isset($_GET["id_room"])){
    ...
    $sql_items="SELECT * FROM `roms` where `id`=$id_room";
    $result=$conn->query($sql_items);
    $result_all=$result->fetch_all();

    echo' <span>Nhập mô tả mới</span>';
    echo ' <textarea name="des" placeholder="Nhập vào mô tả .." cols="20" rows="5">'.$result_all[0][0].'</textarea>';
    echo '     <span>Thay đổi ảnh</span>';
    echo '     <input name="img-room" type="file">';
