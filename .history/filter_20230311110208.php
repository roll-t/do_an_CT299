...
if(isset($_GET["sort_product_detail"])){
  ...
  $sql_product="SELECT * FROM `sanpham` as a JOIN `phong` as b on a.P_ID=b.P_ID  WHERE a.DM_ID= $cate_id and b.P_ID=$rooms_id limmi";
  ...
  function show_list($arr){
    if(count($arr)>0){  
      ...
      if(count($arr)>10){
        ...
        echo '   <a href="./filter.php?next-page=1" class="next"><ion-icon name="chevron-back-outline"></ion-icon></a>';
        echo '  <a href="./filter.php?prve-page=1" class="next"><ion-icon name="chevron-forward-outline"></ion-icon></ion-icon></a>';
        echo '   </div>';
        }
