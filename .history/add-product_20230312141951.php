...
function confirm_change_info_product(){
    ...
    if(isset($_POST["change_info_product"])){
       ...
       if($change_info_ok){
        $sql_insert_product="UPDATE `sanpham` SET  `DM_ID` = '17' , `P_ID` = '8' ,`SP_TEN` = '".$name_product."', `SP_GIA` = '".$price_product."',
         ...
         if($resul_update==true){
           var_dump( $arr_img["DM_ID"] , $arr_img["P_ID"]);
           var_dump( $cate_id , $rooms_id);
           var_dump($_POST["id-cate"],$_POST["id-rooms"]);
            $message="thêm sản phẩm thành công ";
            echo "<script type='text/javascript'>alert('$message');</script>";
            // header('Refresh: 0.1;url=admin.php');
         }
