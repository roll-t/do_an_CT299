...
foreach($arr_render_menu as $items){
    $items_catalog_list=select_value_all("SELECT * FROM `DANHMUC` AS A,`NHOM_DANHMUC` AS B WHERE  A.NDM_ID=B.NDM_ID AND A.DM_ID=$items");
    array_push($arr_catalog_list,implode(",",$items_catalog_list));
}
