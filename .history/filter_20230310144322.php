...
if(isset($_GET["sort_product_detail"])){
  ...
  for($i=0;$i<= count($arr_product);$i++){
    ...
    // echo ' <li class="items-product-select"> ';
    // echo '  <div class="img"><img src="./images/collection/arrivals4.png" alt=""></div>';
    // echo '  <div class="name">'.$i[3].'</div>';
    // echo '  <div class="price">'.$i[4].'</div>';
    // echo '  <div class="remove">';
    // echo '      <a href="./product/update_product.php?id_detail='.$i[0].'" class="update_detail">Cập nhật chi tiết sản phẩm </a>';
    // echo '      <a href="./product/update_product.php?id_change='.$i[0].'" name="change" class="change">Thay đổi</a>';
    // echo '      <a href=""><ion-icon name="trash"></ion-icon></a>';
    // echo '  </div>';
    // echo '</li>';
  }
  if(count($arr_product)>10){
    echo '   <div class="btn-page">';
    echo '   <div class="next"><ion-icon name="chevron-back-outline"></ion-icon></div>';
    echo '   <div class="prve"><ion-icon name="chevron-forward-outline"></ion-icon></div>';
    echo '   </div>';
    }
...
if(isset($_GET["see-all-product"])){
  $sql_all="SELECT * FROM `sanpham` where 1";
  $result_pd_all=$conn->query($sql_all);
  $arr_pd_all=$result_pd_all->fetch_all();
  if(count($arr_pd_all)>0){
    foreach($arr_pd_all as $items){
    echo ' <li class="items-product-select"> ';
    echo '  <div class="img"><img src="./images/collection/arrivals4.png" alt=""></div>';
    echo '  <div class="name">'.$items[3].'</div>';
    echo '  <div class="price">'.$items[4].'</div>';
