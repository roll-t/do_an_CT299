...
if(isset($_GET["value_search"])){
  ...
  $sql_product="SELECT * FROM `sanpham` where `SP_TEN` LIKE '%".$result_search."%' ";
...
function show_list($arr){
  ...
  if($page_items>=4){
      echo '      <a href="./pagin-admin.php?page=1" class="page-items page-per"><ion-icon name="chevron-back-outline"></ion-icon></a>';
  }
  $quantity_paging=$page_items>3?3:$page_items;
  if($page_cate){
      for($i=1;$i<=$quantity_paging;$i++){
        echo '      <a href="./paging.php?page='.$i.'&cate_id='.$id_page.' " class="page-items page-index">'.$i .'</a>';
              }
  }else{
    for($i=1;$i<=$quantity_paging;$i++){
            echo '      <a href="./pagin-admin.php?page='.$i.'" class="page-items page-index">'.$i .'</a>';
                   }
  }
  if($page_items>=4){
      echo '      <a href="./pagin-admin.php?page='.($count_index).'" class="page-items page-next"><ion-icon name="chevron-forward-outline"></ion-icon></a>';
  }
