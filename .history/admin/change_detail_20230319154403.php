...
function add_detail(){
    ...
    if(isset($_POST['product_detail_change'])){
        ...
        foreach($size as $row){
           ...
           function render_list_update(){
               $count=0;
               $arr_img_des=[];
               for($j=0;$j<=4;$j++){
                       if(isset($_FILES["img_des_$count"])){
                           if(check_img("img_des_$count")){
                               $arr_img_des[$count]=target_img($_FILES["img_des_$j"]);
                           }else{
                               alert("Lỗi file ảnh !");
                               break;
                           }
                  ...
                  return  $arr_img_des[$j];
           }
           $arr_img_change= render_list_update();
           var_dump($arr_img_change);
