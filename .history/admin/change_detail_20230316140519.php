...
function add_detail(){
    ...
    if(isset($_POST['product_detail_change'])){
        ...
        if($upload_detail_ok){
            $sql="UPDATE `chitiet_sanpham` SET `CHL_ID` = '".$material."', `MS_ID` = '".$color."', `CSP_IMG_1` ='".target_img($arr_img[1])."', `CSP_KICHTHUOC` = 
            '".$arr_size."', `CSP_IMG_2` = '".target_img($arr_img[2])."', `CSP_IMG_3` ='".target_img($arr_img[3])."', `CSP_IMG_4` = '".target_img($arr_img[4])."', 
            `CSP_MOTA` = '".$des_detail."' WHERE `chitiet_sanpham`.`SP_ID` = 2";
        }
    }
