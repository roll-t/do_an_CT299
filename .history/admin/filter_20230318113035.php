...
$start_from = ($current_page - 1) * $count_index;
  ...
  if($page_cate){
      for($i=1;$i<=$quantity_paging;$i++){
        echo '      <a href="./paging-admin.php?page='.$i.'&cate_id='.$id_page.' " class="page-items page-index">'.$i .'</a>';
              }
  }else{
    for($i=1;$i<=$quantity_paging;$i++){
  ...
  if($page_items>=4){
      echo '      <a href="./paging-admin.php?page='.($count_index).'" class="page-items page-next"><ion-icon name="chevron-forward-outline"></ion-icon></a>';
  }
