...
if(isset($_GET["id_product_add"])){
  ...
  if(isset($_GET["see-all-product"])){
    ...
    if($room_id){
      $sql="SELECT * FROM `SANPHAM` AS A,`PHONG` AS B LIMIT $start_from, $count_index WHERE A.P_ID=B.P_ID AND A.P_ID=$room_id and A.DM_ID=$cate_id ";
    }else{
      $sql="SELECT * FROM `SANPHAM` LIMIT $start_from, $count_index WHERE DM_ID=$cate_id";
    }
    $arr_pd=select_value_all($sql);
    show_list($arr_pd);
  }
