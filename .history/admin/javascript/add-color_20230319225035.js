...
const btn_delete_color=document.querySelectorAll(".body-material .list-color .delete-color ion-icon")
const id_material=document.querySelector(".id_material").value
btn_delete_color.forEach(items=>{
    items.addEventListener("click",e=>{
        const link=items.parentElement
        const text="Bạn muốn xóa chất liệu này ?";
        if(confirm(text)){
            link.href=`./add-color-material.php?delete_material=${id_material}`;
        }
    })
})
