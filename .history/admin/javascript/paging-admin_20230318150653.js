...
function paging(){
    ...
    let curent_index=0;
    let link_target=`./paging-admin.php?page=`
    let id=[];
    let change=false;
    ...
    btn_next && btn_next.addEventListener("click",e=>{
        if(curent_index==2){
            ...
            btn_next.href=`${link_target}${count+2}&all_id`;
            btn_per.href=`${link_target}${count-2}&all_id`
            list_page.forEach((items)=>{
                items.innerHTML=index;
                items.href=`${link_target}${index}&all_id`;
                index++;
            })
        }else{
            curent_index++;
    ...
    btn_per && btn_per.addEventListener("click",e=>{
        if(curent_index===0){           
            ...
            btn_next.href=`${link_target}${count+1}&all_id`;
            btn_per.href=`${link_target}${count-3}&all_id`
            if(count==3){
            ...
            list_page.forEach(items=>{
                ...
                items.href=`${link_target}${index-2}&all_id`
                index++;
            })
        }else{
            curent_index--;
