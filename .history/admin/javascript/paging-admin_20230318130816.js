...
function paging(){
    ...
    let curent_index=0;
    let target=`./paging-admin.php?page=`
    let count=2;
    ...
    btn_next && btn_next.addEventListener("click",e=>{
        if(curent_index==2){
            ...
            btn_next.href=`./paging-admin.php?page=${count+2}`;
            btn_per.href=`./paging-admin.php?page=${count-2}`
            list_page.forEach((items)=>{
                items.innerHTML=index;
                items.href=`./paging-admin.php?page=${index}`;
                index++;
            })
        }else{
            curent_index++;
    ...
    btn_per && btn_per.addEventListener("click",e=>{
        if(curent_index===0){           
            ...
            btn_next.href=`./paging-admin.php?page=${count+1}`;
            btn_per.href=`./paging-admin.php?page=${count-3}`
            if(count==3){
            ...
            list_page.forEach(items=>{
                ...
                items.href=`./paging-admin.php?page=${index-2}`
                index++;
            })
        }else{
            curent_index--;
