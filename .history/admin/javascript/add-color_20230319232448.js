...
btn_change_material.forEach(items=>{
    items.addEventListener("click",e=>{
      ...
      input.innerHTML=""
      input.append(tag);
      document.querySelector(".link_change").addEventListener("click",e=>{
        console.log(document.querySelector(".link_change"))
          if(document.querySelector(".value_change").value!=""){
            document.querySelector(".link_change").href=`./add-color-material.php?id_change=${id_material[index].value}`;
          }
      })
  })
})