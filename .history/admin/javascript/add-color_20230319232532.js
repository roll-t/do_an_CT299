...
btn_change_material.forEach(items=>{
    items.addEventListener("click",e=>{
      ...
      document.querySelector(".link_change-material").addEventListener("click",e=>{
        console.log(document.querySelector(".link_change-material"))
          if(document.querySelector(".value_change").value!=""){
            document.querySelector(".link_change-material").href=`./add-color-material.php?id_change=${id_material[index].value}`;
          }
      })
  })
})