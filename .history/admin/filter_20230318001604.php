...
//   $arr_pd_all=select_value_all($sql_search);
...
function filter(){
  global $start_from, $results_per_page;
  if(isset($_GET["see-all-product"])){
    $sql="SELECT * FROM `sanpham`
    LIMIT $start_from, $results_per_page";
    $arr_pd=select_value_all($sql);
  }else if(isset($_GET["see-all-product"])){
      $value_search=$_GET["confirm_search"];
      $sql="SELECT * FROM `sanpham`  
      WHERE SP_TEN LIKE '%".$value_search."%' LIMIT $start_from, $results_per_page";
      $arr+pd=select_value_all($sql_search);
  }
}
filter();
?>