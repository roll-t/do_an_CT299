...
function add_detail(){
    ...
    if(isset($_POST['product_detail_change'])){
        ...
        var_dump($arr_img_detail,$arr_img);
        for($i=0;$i<count($arr_img);$i++){
            if(!check_img($arr_name_img[$i])){
               $arr_link=$arr_img_detail[$i];
            }else{
        ...
        if($upload_detail_ok){
            $sql="UPDATE `chitiet_sanpham` SET `CHL_ID` = '".$material."', `MS_ID` = '".$color."', `CSP_IMG_1` ='".target_img($arr_img[0])."', `CSP_KICHTHUOC` = 
            '".$arr_size."', `CSP_IMG_2` = '".target_img($arr_img[1])."', `CSP_IMG_3` ='".target_img($arr_img[2])."', `CSP_IMG_4` = '".target_img($arr_img[3])."', 
