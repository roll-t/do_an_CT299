...
function add_detail(){
    ...
    if(isset($_POST['product_detail_change'])){
        ...
        $img_ok=false;
        foreach($arr_img_des as $items){
            check_img($items) ? $img_oke=true :$img_ok=false;
            if(!$img_ok){
                alert("Lỗi file ảnh !");
                break;
            }
        ...
        if($upload_detail_ok &&  $img_oke){
            $sql="UPDATE `chitiet_sanpham` SET `SP_ID` = '20', `CHL_ID` = '6', `CSP_KICHTHUOC` = '200,500,600', `MS_ID` = '2', `CSP_MOTA` = 'hello', `CSP_DS_IMG` = './images/list-product/6-khay-gom-matt-grey-calligaris-23-768x511.jpg,./images/list-product/6-Binh-Hoa-Sianni-M-Blue-114703E-3105573-1-768x454.jpg,./images/list-product/5-Binh-Hoa-Sianni-M-Blue-114703E-3105573-768x454.jpgg' WHERE `chitiet_sanpham`.`SP_ID` = 50;"
        }
    }
