...
include '../query.php';
 ...
 if(isset($_GET["cate_id"])){
    ...
    if($room_id){
       $sql_count="SELECT COUNT(*) AS total FROM `SANPHAM` AS A,`PHONG` AS B WHERE A.P_ID=B.P_ID AND A.P_ID=$room_id and A.DM_ID=$cate_id ";
    }else{
       $sql_count="SELECT * FROM `SANPHAM` WHERE DM_ID=$cate_id";
    }
 }else{
