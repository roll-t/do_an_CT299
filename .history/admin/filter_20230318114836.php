...
session_start();
  ...
  if(isset($_GET["see-all-product"])){
    ...
    show_list($arr_pd);
      ...
      WHERE SP_TEN LIKE '%".$value_search."%' LIMIT $start_from, $count_index";
      $arr_pd=select_value_all($sql);
  }
    ...
    WHERE a.DM_ID= $cate_id and b.P_ID=$rooms_id LIMIT $start_from, $count_index";
    }else{
      $sql="SELECT * FROM `sanpham`  
      WHERE DM_ID= $cate_id LIMIT $start_from, $count_index";
    }
  $arr_pd=select_value_all($sql);
  show_list($arr_pd);
  }
  ...
  if($page_items>=4){
      echo '      <a href="./filter.php?page=1" class="page-items page-per"><ion-icon name="chevron-back-outline"></ion-icon></a>';
  }
  $quantity_paging=$page_items>3?3:$page_items;
  if($page_cate){
      for($i=1;$i<=$quantity_paging;$i++){
        echo '      <a href="./filter.php?page='.$i.'&cate_id='.$id_page.' " class="page-items page-index">'.$i .'</a>';
              }
  }else{
    for($i=1;$i<=$quantity_paging;$i++){
            echo '      <a href="./filter.php?page='.$i.'" class="page-items page-index">'.$i .'</a>';
                   }
  }
  if($page_items>=4){
      echo '      <a href="./filter.php?page='.($count_index).'" class="page-items page-next"><ion-icon name="chevron-forward-outline"></ion-icon></a>';
  }
