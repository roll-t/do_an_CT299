...
include '../query.php';
 ...
 if($page_cate_true){
    for($i=1;$i<=$quantity_paging;$i++){
      echo '      <a href="./paging-admin.php?page?page='.$i.'&cate_id='.$cate_id.' " class="page-items page-index">'.$i .'</a>';
            }
}else{
  for($i=1;$i<=$quantity_paging;$i++){
          echo '      <a href="./paging-admin.php?page?page='.$i.'" class="page-items page-index">'.$i .'</a>';
                 }
}
