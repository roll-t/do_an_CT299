...
function select_address(index){
    ...
    const list_select=body_product.querySelectorAll(".address-save .wrap-input select")
    const catalog_list=body_product.querySelectorAll(".address-save .wrap-input select")[0]
    const category=body_product.querySelectorAll(".address-save .wrap-input select")[1]
    const rooms=body_product.querySelectorAll(".address-save .wrap-input select")[2]
    const address_1=document.querySelectorAll(".addrees_1")[0]
    list_select.forEach((items,index)=>{
        items.addEventListener("change",e=>{
            const get_value=()=>(items.value);
            btn_filter.href=`filter.php?id_room=${get_value()}`
