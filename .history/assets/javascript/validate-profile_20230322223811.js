...
function check_fill(input,text){
    let is_true=true
    input.value=="" && show_error(input,text) && (is_true=false);
    return is_true
}
...
function check_address(input,text){
    ...
    select.value=="" && show_error(select,text) && (is_true=false)
    return is_true;
}
...
btn_save.addEventListener("click",e=>{
    console.log(check_fill(name_client,"Không được bỏ trống Tên"))
    check_fill(name_client,"Không được bỏ trống Tên")?
    check_address(list_input[2],"không được bỏ trống Tỉnh"):false;
