...
$(".link-check-password-login").click(function(){
    let url=`./client/check-name-login.php?password_login=${password_login.value}`
    this.href=url;
...
$(".link_confirm_password").click(function(){
    let url=`./client/check-name-login.php?confirm_password=${document.querySelector(".id_password_login").value}&input_password=${password_login.value}`
    this.href=url;
    $(".error_password").load(url);
    return false;
...
document.querySelector(".login_sucess")!=null && setInterval(()=>{
    btn_login.click();
},100)