...
const gender =list_input[1].querySelectorAll("input")
console.log(gender)
let value_gender=" ";
gender.forEach(items=>{
    items.addEventListener("click",e=>{
        value_gender=items.value;
    })
})
if(name_client){
    $(".btn_confirm_info").click(function(){
        this.href=`./profile/add-name-gender.php?add_name=${get_name()}&add_gender=${value_gender}`
        let url=this.href
        $(".wrap_name").load(url)
        return false
    })
...
if(value_gender!=""){
    $(".btn_confirm_info").click(function(){
        this.href=`./profile/add-name-gender.php?add_gender=${value_gender}`
        let url=this.href
        $(".group-input.gender p").load(url)
        return false
    })
}