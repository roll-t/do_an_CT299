...
function preview(){
    const btn=document.querySelector(".preview .right-side-add .preview");
    btn.addEventListener("click",e=>{
        const name_pd=document.querySelectorAll(".left-side-add .wrap-input input")[0]
        const price_pd=document.querySelectorAll(".left-side-add .wrap-input input")[1]
        ...
        localStorage.setItem("preview_product",JSON.stringify([name_pd,price_pd,img_1_pd,img_2_pd]))
    })
    $(".info-product-add .right-side-add .preview").click(function(){
        let url=this.href;
        $(".info-product-add .right-side-add .product-items").load(url)
        return false;
    })
}
preview()
