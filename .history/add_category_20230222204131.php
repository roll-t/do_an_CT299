<?php
        ...
        if(isset($_POST["add-room"])){
            ...
            $target_dir = "images/room/banner-room";
            $target_file = $target_dir . basename($_FILES["img-room"]["name"]);
            $uploadOk = 1;
            ...
            $sql="INSERT INTO `rooms` (`id`, `".$_POST_["name-room"]."`, `".$_POST["des"]."`, `image_path`) VALUES (NULL, 'Phòng Khách', 'Phòng này của tao', './img/img.png')";
            $result=$conn->query($sql);
            if($result==true){
                header("Location: admin.php ");
            }
