<?php
        ...
        if(isset($_POST["add-room"])){
            ...
            if($uploadOk){
                ...
                if(!empty($_POST["name-room"]) && !empty($_POST["name-room"]) && !empty($_FILES["img-room"]["name"])){
        ...
        if(isset($_POST["change-room"])){
            ...
            if($uploadOk){
                $sql="UPDATE `rooms` SET `name` = '".$_POST["name-room"]."', `description` = '".$_POST["des"]."', `image_path` = '".$target_file."' WHERE `rooms`.`id` = '".$_POST["select_value"]."' ";
                $result=$conn->query($sql);
                if($result==true){
                    header("Location: admin.php ");
                }
