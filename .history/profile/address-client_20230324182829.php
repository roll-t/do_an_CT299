...
if(isset($_GET["add_address"])){
    ...
    $sql="INSERT INTO `diachi` (`DC_ID`, `KH_ID`, `T_ID`, `DC_MOTA`, `DC_HUYEN`) VALUES (NULL, '".$id_client."', '".$info_address[0]."', 'hello', '".$name_huyen["H_TEN"]."')";
    $result=$conn->query($sql);
    $name_tinh=select_value("SELECT T_TEN FROM TINH WHERE T_ID=$info_address[0]");
    $main_address=[$name_tinh["T_TEN"],$name_huyen["H_TEN"]];
    if($result){
        setcookie("address_homedesign", implode(",", $info_address), time() + (86400 * 30), "/");
