...
<script>
    localStorage.setItem('email', '<?php echo $_SESSION['email']="my email";?>');  
    localStorage.setItem('password', '<?php echo $_SESSION['password']="my password";?>');
</script>
...
<?php
    ...
    $keys = "<script>document.write(localStorage.getItem('key_select'));</script>";
            ...
            <div class="body-add-category">
                <form action="./add_category.php" method="post" class="group-input">
                    <div class="wrap-input">
                       <span>Chọn danh sách danh mục: </span>
                       <?php 
                        echo '<select name="value_select" class="select_catalog_list">';
                        foreach($result_all as $row){
                            echo "<option value=".$row[0].">".$row[1]."</option>";
                        }
                        ?>
                    ...
                    <div class="wrap-input">   
                       <select name="category_id" class="detele-catalog-list">
                       <?php           
                       $key_select = "<script>document.write(localStorage.getItem('key_select'));</script>";
                       $id=isset($key_select)?15:$key_select;
                       $sql_ss="SELECT * FROM `product_categories` as a JOIN `catalog_list` as b WHERE a.id_catalog_list=b.id and a.id_catalog_list=id";
                       
                       $result_menu=$conn->query($sql_ss); 
                       $menu_items=$result_menu->fetch_all();
