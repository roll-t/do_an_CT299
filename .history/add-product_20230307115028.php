...
if(isset($_POST["confirm_add_product"])){
    ...
    function insert_product(){
        $cate_id=$_POST["cate_id_add_product"];
        $room_is=$_POST["room_id_add_product"];
        ...
        if(check_img("img_1")){
            ...
            $target_file = $target_dir . basename($img_1["name"]);
        }
        if(check_img("img_2")){
            $target_dir = "images/collection/";// đường dẫn tới file upload
            $target_file = $target_dir . basename($img_2["name"]);
        }
       $sql=" INSERT INTO `sanpham` (`SP_ID`, `DM_ID`, `P_ID`, `SP_TEN`, `SP_GIA`, `SP_IMG_1`, `SP_IMG_2`) VALUES (NULL, '11', '4', 'sản phẩm 1', '122334', './img/local', './img/local') ";
